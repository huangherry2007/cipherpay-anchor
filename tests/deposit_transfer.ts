...
// ───────────────────────── tests ─────────────────────────
describe("deposit → transfer (strict sync end-to-end)", () => {
    ...
    try {
      await program.methods
        ...
        .accounts({
          ...
          authority: payer,
          // Do not pass systemProgram; IDL pins it by address.
        })
        .signers([rootCache])
        .rpc();
  ...
  it("end-to-end: deposit then transfer (strict sync)", async () => {
    // -------------------- DEPOSIT --------------------
    ...
    const depIx = await program.methods
      // IMPORTANT: pass Buffers directly for bytes args
    ...
    const curIdx: number = treeAfterDeposit.nextIndex;
    console.log(
    ...
    // -------------------- TRANSFER --------------------
    const nullifier = slice32(xferPublics, TRANSFER_IDX.NULLIFIER);
    const spentRoot = slice32(xferPublics, TRANSFER_IDX.MERKLE_ROOT);
    console.log("🔒 nullifier (LE, hex):", toHexLE(nullifier));
    ...
    const xferSig = await program.methods
      // nullifier: [u8;32] → number[] ; bytes args → Buffers
      .shieldedTransfer(Array.from(nullifier), xferProof, xferPublics)
      .accountsPartial({
        tree: treePda,
        rootCache: rootCache.publicKey,
    ...
    // Optional: logs
    const res = await connection.getTransaction(xferSig, {
      commitment: "confirmed",
    ...
    console.log(
      "🌳 tree after transfer — nextIndex=",
      treeAfterTransfer.nextIndex
    );
  });

  it("sanity: proof/publics lengths", () => {
    expect(depProof.length).toBe(256);
    expect(depPublics.length).toBe(7 * 32);
    expect(xferProof.length).toBe(256);
    expect(xferPublics.length).toBe(9 * 32);
